...
|------|------|------|------|------|------|------|------|	
| 1 | 加入购物车 | add_to_cart | 用户将商品加入购物车的行为 | 事务事实 | 每一行代表一次用户将某商品加入购物车的行为 | 购物车ID + 商品ASIN + 时间戳 | 事件时间 |	
| 2 | 创建订单 | create_order | 用户提交订单的行为 | 事务事实 | 每一行代表一笔订单的创建事件 | 订单ID | 订单创建时间 |	
| 3 | 订单明细 | order_detail | 订单中每个商品明细 | 事务事实 | 每一行代表一笔订单中的一个商品明细 | 订单ID + 订单明细行号 | 订单创建时间 |	
| 4 | 支付订单 | pay_order | 用户完成订单支付 | 事务事实 | 每一行代表一笔订单的一次支付事件 | 订单ID + 支付ID | 支付时间 |	
| 5 | 结算订单 | settle_order | 平台完成订单结算 | 事务事实 | 每一行代表一笔订单的一次平台结算记录 | 订单ID + 结算ID | 结算时间 |	
| 6 | 发布评价 | submit_review | 用户对订单发表评价 | 事务事实 | 每一行代表一条用户评价记录 | 评价ID | 评价发布时间 |	
